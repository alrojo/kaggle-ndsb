...
#    assert False
	...
	num = "@" + str(e) + "@"
	my_metadata_tmp_path = metadata_tmp_path[:-4] + num + metadata_tmp_path[-4:]
	my_metadata_target_path = metadata_target_path[:-4] + num + metadata_target_path[-4:]
        with open(my_metadata_tmp_path, 'w') as f:
            pickle.dump({
        ...
        copy_process = Popen(['cp', my_metadata_tmp_path, my_metadata_target_path])
        print "  saved to %s, copying to %s" % (my_metadata_tmp_path, my_metadata_target_path)
        print
